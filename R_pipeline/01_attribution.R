...
expected_domain <- demography[, .(year, agegroup)]
expected_domain <- CJ(year = sort(unique(expected_domain$year)), agegroup = c("65-74", "75-84", "85+"))
...
obs_clim <- obs_city[year %in% hist_years_local, .(tmean_hist = mean(tmean_obs, na.rm = TRUE)), by = month_day]
if (anyNA(obs_clim$tmean_hist)) stop("Historical climatology for Madrid contains NA values.", call. = FALSE)
...
hist_sim <- tmean_all[ssp == "hist" & year %in% hist_years_local]
...
obs_hist <- obs_city[year %in% hist_years_local]
hist_clim_md <- obs_hist[, .(
  tmean_hist = mean(tmean_obs, na.rm = TRUE)
), by = month_day]
...
for (variant in variant_levels) {
  ...
  if (variant == "with_cc") {
    ...
    t_work <- merge(t_work, hist_clim_md, by = "month_day", all.x = TRUE, sort = FALSE)
    t_work[, tmean_variant := tmean_hist]
    t_work[, temp_branch := "without_cc"]
    t_work[, tmean_hist := NULL]
  }
  ...
  for (agegrp in c("65-74", "75-84", "85+")) {
    ...
    t_work <- merge(t_work, death_annual, by = "year", all.x = TRUE, sort = FALSE)
    if (anyNA(t_work$death)) stop(sprintf("Missing annual deaths for %s/%s.", city_id, agegrp), call. = FALSE)

    range_idx <- fcase(
      t_work$tmean_variant < p2_5, "ExtrCold",
      t_work$tmean_variant < mmt, "ModCold",
      t_work$tmean_variant < p97_5, "ModHeat",
      default = "ExtrHeat"
    )

    b_fut <- onebasis(t_work$tmean_variant, fun = varfun, degree = vardegree, knots = knots, Bound = bound)
    ...
    annual <- data.table(
      ...
      an = an_daily,
      branch = variant,
      days_in_year = t_work$days_in_year
    )
...
full_domain <- CJ(branch = variant_levels, year = sort(unique(demography$year)), agegroup = c("65-74", "75-84", "85+"), range = range_levels)
