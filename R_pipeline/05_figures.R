...
#
# R Pipeline Step 05: Summary figures and headline numbers
#   Built only from Step 04 outputs. The figures Simon asked for:
...
city_name <- "Madrid"
scenario_label <- "SSP3-7.0, GFDL-ESM4, central ERF estimates"
...
dle <- fread(file.path(out_dir, "04_le_decomposition.csv"))
dli <- fread(file.path(out_dir, "04_li_decomposition.csv"))
...
agg_block <- function(d, measure) {
  d <- d[cause %in% range_levels]
  d[, block := cut(year_to, time_blocks, right = TRUE, include.lowest = TRUE,
    labels = paste(head(time_blocks, -1), tail(time_blocks, -1) - 1, sep = "-"))]
  d[, age_band := cut(age, age_band_breaks, right = FALSE,
    labels = c(paste(head(age_band_breaks, -2), head(age_band_breaks, -2) + 4, sep = "-"), "100+"))]
  d[, .(contribution = sum(contribution)), by = .(branch, block, age_band, cause)][, measure := measure]
}
blk <- rbind(agg_block(copy(dle), "LE65 (years)"), agg_block(copy(dli), "LI65+ (SD)"))
...
p2 <- ggplot(blk, aes(age_band, contribution, fill = cause)) +
  ...
  labs(title = sprintf("%s: contribution of temperature-related mortality to the change in LE65 and LI65+", city_name),
       subtitle = paste(scenario_label, "- summed over 20-year blocks; all other causes not shown"),
