...
#
# Adapted from Masselot et al. 2025
  ...
  perlen = 1, ensonly = T, warming_win = NULL)
{
  ...
  setnafill(res, fill = 0, cols = intersection(measures, names(res)))
  ...
  measures = c("af", "rate", "cuman"), by = key(res))
{
  ...
  setnafill(res, fill = 0, cols = intersection(newvars, names(res)))
...
impact_summarise <- function(res, vars = "an", by = by, prob = .95){
...
fquantile <- function(x, p, ...) quantile(x, p, ...)
