...
Attribution units show that the raw daily AF×deaths accumulation is about 365× the annualized result, but the Masselot comparison exceeds the explicit 1e-6 threshold, so the attribution-units gate fails on this fixture. The stratified-sample diagnosis and counterfactual tests point to a genuine Masselot reference mismatch rather than a unit, leap-year, coefficient-order, or centering error.
...
The remaining unresolved blockers are the broader negative grouped-AN census, PCLM zero/NA behavior, and IPF summary-column behavior. The current single-age baseline file provenance is fixed-baseline PCLM and is not yet an SSP-varying demographic-weight source.
